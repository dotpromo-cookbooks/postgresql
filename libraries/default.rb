  ...
  module PostgresqlHelpers
    ...
    def pg_tzdir
      ...
      if ::File.directory?('/usr/lib/zoneinfo')
        ...
        return share_path if ::File.directory?(share_path)
      end
      nil
    end
    ...
    def scan_available_timezones(tzdir)
      ...
      if tzdir.nil?
        ...
        bestzonename = ::File.readlink('/etc/localtime').gsub("#{tzdir}/", '')
        ...
        Find.find(tzdir) do |path|
          ...
          next if ::File.basename(path) == 'posixrules' || ::File.basename(path) == 'localtime'
    ...
    def pgdgrepo_rpm_info
      ...
      [repo_rpm_url, repo_rpm_filename, repo_rpm_package]
    end
