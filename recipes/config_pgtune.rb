...
config_pg_tune_available = node['postgresql'].attribute?('config_pgtune')
config_pg_tune = node['postgresql']['config_pgtune'] if config_pg_tune_available

if config_pg_tune_available && config_pg_tune.attribute?('db_type')
  db_type = config_pg_tune['db_type']
...
if config_pg_tune_available && config_pg_tune.attribute?('max_connections')
  max_connections = config_pg_tune['max_connections']
...
if config_pg_tune_available && config_pg_tune.attribute?('total_memory')
  total_memory = config_pg_tune['total_memory']
...
if mem >= 256
  ...
  node.default['postgresql']['config']['shared_buffers'] = binaryround(shared_buffers * one_mega)
  ...
  node.default['postgresql']['config']['effective_cache_size'] = binaryround(effective_cache_size * one_mega)
  ...
  node.default['postgresql']['config']['work_mem'] = binaryround(work_mem * one_mega)
  ...
  node.default['postgresql']['config']['maintenance_work_mem'] = binaryround(maintenance_work_mem * one_mega)
end
